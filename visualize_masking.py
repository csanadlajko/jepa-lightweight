...
import numpy as np
from src.IJEPA.mask.masking import Mask, apply_mask, CellMask
from src.IJEPA.transform.datatransform import get_cifarten_dataset, get_pdl1_dataset
...
pdl1_train_loader, pdl1_test_loader = get_pdl1_dataset(args.dataset_input, args.annotation_path, args.reverse_transform)
...
def visualize_masks(images, context_masks, target_masks, num_samples=4):
    ...
    for i in range(batch_size):
        ...
        if isinstance(target_masks[i], list):
            for block_idx in target_masks[i]:
                if isinstance(block_idx, torch.Tensor) and block_idx.numel() > 0:
                    row = int(block_idx.item()) // num_patches_per_side
                    col = int(block_idx.item()) % num_patches_per_side
                    if 0 <= row < num_patches_per_side and 0 <= col < num_patches_per_side:
        ...
        axes[i, 0].imshow(img)
        axes[i, 0].set_title(f'Original cell image')
        axes[i, 0].axis('off')
        ...
        axes[i, 1].set_title(f'Non-masked cells (context)')
        axes[i, 1].axis('off')
        ...
        axes[i, 2].set_title(f'Masked cells (20% of total)')
        axes[i, 2].axis('off')
...
if __name__ == "__main__":
    mask = Mask() ## used for regulat I-JEPA masking

    
    for images, annotations in pdl1_test_loader:
        images = images.cuda() if torch.cuda.is_available() else images

        cell_mask = CellMask() ## used for cell masking

        context_masks, target_masks = cell_mask(20, annotations["boxes"])
